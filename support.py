...
CSS = '''
div.sub-notify { padding: 10px; margin: 0; }
.sub-notify h1, .sub-notify h2, .sub-notify h3,
.sub-notify h4, .sub-notify h5, .sub-notify h6 {
    {{'.string'|css}}
}
...
class SubNotifySupportInfoCommand(sublime_plugin.ApplicationCommand):
    """Support info."""
...
class SubNotifyOpenSiteCommand(sublime_plugin.ApplicationCommand):
    """Open site links."""
...
class SubNotifyDocCommand(sublime_plugin.WindowCommand):
    """Open doc page."""
...
class SubNotifyChangesCommand(sublime_plugin.WindowCommand):
    """Changelog command."""
    ...
    def run(self):
        ...
        view = self.window.new_file()
        view.set_name('SubNotify - Changelog')
        view.settings().set('gutter', False)
        view.settings().set('word_wrap', False)
        if has_phantom_support:
            mdpopups.add_phantom(
